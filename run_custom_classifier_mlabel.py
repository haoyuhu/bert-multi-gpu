...
flags.DEFINE_integer(
    "num_labels", 10,
    "Number of labels in dataset for multi-label classification")
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, labels=None):  # change from label to labels
...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.label_ids = label_ids  # change
...
class MlabelProcessor(DataProcessor):
    """added for multi-label classification"""
    ...
    def _create_examples(self, lines, set_type):
        # idx, text_a, label_0, label_1, ...
        examples = []
        for (i, line) in enumerate(lines):
            if i == 0:
                continue  # all the files have header and skip header here
            guid = "%s-%s" % (set_type, i)
...
def convert_single_example(ex_index, example, max_seq_length, tokenizer):
    ...
    if ex_index < 5:
        ...
        tf.logging.info("label: %s (id = %s)" % (example.labels, label_ids))
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
    ...
    def input_fn(params):
        ...
        if is_training:
            d = d.repeat()
            d = d.shuffle(buffer_size=500)  # change from 100 to 500
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 label_ids, use_one_hot_embeddings, fp16):
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, probabilities, is_real_example):
                ...
                preds_split = tf.cast(tf.greater(probs_split, tf.constant(0.5)), tf.int32)
                ...
                for idx in range(NUM_LABELS):
                    ...
                    acc, acc_op = tf.metrics.accuracy(
                        labels=label_ids_spec, predictions=preds_spec, weights=is_real_example)
                    ...
                    eval_dict["eval_f1score_" + str(idx)] = (f1, tf.identity(f1))

                eval_dict["eval_loss"] = tf.metrics.mean(values=per_example_loss, weights=is_real_example)
...
def save_for_serving(estimator, serving_dir, seq_length, is_tpu_estimator):
    feature_map = {
        ...
        "label_ids": tf.placeholder(tf.int32, shape=[None, NUM_LABELS], name='label_ids'),
    }
...
def main(_):
    ...
    global NUM_LABELS
    NUM_LABELS = FLAGS.num_labels  # global variable

    processors = {
        "mlabel": MlabelProcessor
    }
